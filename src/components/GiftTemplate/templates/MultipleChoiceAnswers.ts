...
type MultipleChoiceAnswerOptions = TemplateOptions &
  Pick<MultipleChoiceType, "choices">;
...
export default function MultipleChoiceAnswers({
  ...
  const isMultipleAnswer =
    choices.filter(({ isCorrect }) => isCorrect === true).length === 0;
      ...
      return `
        <div>
          ...
          ${AnswerWeight({ correct: isCorrectOption, weight: weight })}
            <label style="${CustomLabel} ${ParagraphStyle(
        state.theme
      )}" for="${inputId}">
