...
const multiple: GIFTQuestion[] = [
  {
    ...
    globalFeedback: {
      format: "moodle",
      text:
        "Not sure? There are many answers for this question so do not fret. Not sure? There are many answers for this question so do not fret."
    },
    choices: [
      {
        isCorrect: false,
...
const items = multiple
  .map((item) => Template(item, { theme: "dark" }))
  .join("");
const errorItemDark = ErrorTemplate("Hello");

const lightItems = multiple
  .map((item) => Template(item, { theme: "light" }))
  .join("");

const errorItem = ErrorTemplate("Hello");

document.getElementById("app").innerHTML =
  items + errorItemDark + lightItems + errorItem;
